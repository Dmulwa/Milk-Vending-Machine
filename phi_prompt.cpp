...
{	
	static uint32_t instant_t5 = 0;
	static uint32_t instant_t6 = 0;
	uint32_t instant_t7 = millis();
	static uint32_t instant_t8 = 0;
    int temp1=0;
    byte render=1, width=para->width;
    while(true)
    {
		//Modification by Ngoima: Querring SMS, Sending SMS and watchdog timer kicks
		//-----------LCD prob.------------
		/* delay(500);
		lcd->begin(16, 4); */
		if(millis() - instant_t7 >= (uint32_t)120000) {
			instant_t7 = millis();
			return -1;
		}
		if(millis() - instant_t6 >= (uint32_t)24000) { //reset watchdog timer after 4 secs
			instant_t6 = millis();
			wdt_reset();
		}
		if(millis() - instant_t8 >= (uint32_t)45000) {
			instant_t8 = millis();
			query_sms();
			//lock_check();
		}
...
int input_number(phi_prompt_struct *para)
{
  ...
  {  
	//Modification by Ngoima: Querring SMS, Sending SMS and watchdog timer kicks
		...
		if(millis() - instant_t6 >= (uint32_t)10000) { //reset watchdog timer after 4 secs
			instant_t6 = millis();
			wdt_reset();
		}
		/* if(millis() - instant_t5 >= (uint32_t)60000) {
			instant_t5 = millis();
			//Serial.println("I am in input_number()!");
			query_sms();
      //lock_check();
			//send_report();
		} */
